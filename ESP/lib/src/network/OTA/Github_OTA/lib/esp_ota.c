...
static void SetFlag(Github_OTA_client_handle_t *handle, enum release_flags flag) { handle->result.flags |= flag; }
static bool GetFlag(Github_OTA_client_handle_t *handle, enum release_flags flag) { return handle->result.flags & flag; }

static void ClearFlag(Github_OTA_client_handle_t *handle, enum release_flags flag) { handle->result.flags &= ~flag; }
...
esp_err_t Github_OTA_free(Github_OTA_client_handle_t *handle) {
    ...
    if (handle->username) free(handle->username);
    if (handle->token) free(handle->token);
...
static void lwjson_callback(lwjson_stream_parser_t *jsp, lwjson_stream_type_t type) {
...
#ifdef DEBUG
    ESP_LOGI(TAG, "Lwjson Called: %d %d %d %d", jsp->stack_pos, jsp->stack[jsp->stack_pos - 1].type, type,
             handle->result.flags);
    if (jsp->stack[jsp->stack_pos - 1].type == LWJSON_STREAM_TYPE_KEY) { /* We need key to be before */
    ...
    if (!GetFlag(handle, Github_OTA_RELEASE_VALID_ASSET) || !GetFlag(handle, Github_OTA_RELEASE_GOT_STORAGE)) {
        if (jsp->stack_pos == 5 && jsp->stack[0].type == LWJSON_STREAM_TYPE_OBJECT &&
            jsp->stack[1].type == LWJSON_STREAM_TYPE_KEY && strcasecmp(jsp->stack[1].meta.name, "assets") == 0 &&
            ...
            ESP_LOGD(TAG, "Assets Got key '%s' with value '%s'", jsp->stack[jsp->stack_pos - 1].meta.name,
                     jsp->data.str.buff);
            ...
            if (GetFlag(handle, Github_OTA_RELEASE_GOT_FNAME) && GetFlag(handle, Github_OTA_RELEASE_GOT_URL)) {
                ESP_LOGD(TAG, "Testing Firmware filenames %s -> %s - Matching Filename against %s and %s",
                         handle->scratch.name, handle->scratch.url, handle->config.filenamematch,
                         handle->config.storagenamematch);
                /* see if the filename matches */
                if (!GetFlag(handle, Github_OTA_RELEASE_VALID_ASSET) &&
                    fnmatch(handle->config.filenamematch, handle->scratch.name, 0) == 0) {
                ...
                } else if (!GetFlag(handle, Github_OTA_RELEASE_GOT_STORAGE) &&
                           fnmatch(handle->config.storagenamematch, handle->scratch.name, 0) == 0) {
                    strncpy(handle->result.storageurl, handle->scratch.url, CONFIG_MAX_URL_LEN);
                    ESP_LOGD(TAG, "Valid Storage Asset Found: %s - %s", handle->scratch.name,
...
esp_err_t Github_OTA_check(Github_OTA_client_handle_t *handle) {
    ...
    ESP_ERROR_CHECK(esp_event_post(Github_OTA_EVENTS, Github_OTA_EVENT_START_CHECK, handle,
                                   sizeof(Github_OTA_client_handle_t *), portMAX_DELAY));
    ...
    if (res != lwjsonOK) {
        ...
        ESP_ERROR_CHECK(esp_event_post(Github_OTA_EVENTS, Github_OTA_EVENT_NOUPDATE_AVAILABLE, handle,
                                       sizeof(Github_OTA_client_handle_t *), portMAX_DELAY));
        xSemaphoreGive(Github_OTA_lock);
        return ESP_FAIL;
    }
    ...
    char url[CONFIG_MAX_URL_LEN];
    snprintf(url, CONFIG_MAX_URL_LEN, "https://%s/repos/%s/%s/releases/latest", handle->config.hostname,
             handle->config.orgname, handle->config.reponame);
    ...
    if (err == ESP_OK) {
        ESP_LOGD(TAG, "HTTP GET Status = %d, content_length = %d", esp_http_client_get_status_code(client),
    ...
    if (esp_http_client_get_status_code(client) == 200) {
        if (GetFlag(handle, Github_OTA_RELEASE_VALID_ASSET)) {
            ...
            ESP_LOGI(TAG, "Current Version %d.%d.%d", handle->current_version.major, handle->current_version.minor,
                     handle->current_version.patch);
            ESP_LOGI(TAG, "New Version %d.%d.%d", handle->latest_version.major, handle->latest_version.minor,
                     handle->latest_version.patch);
    ...
    esp_http_client_cleanup(client);
    ESP_ERROR_CHECK(esp_event_post(Github_OTA_EVENTS, Github_OTA_EVENT_UPDATE_AVAILABLE, handle,
                                   sizeof(Github_OTA_client_handle_t *), portMAX_DELAY));
    xSemaphoreGive(Github_OTA_lock);
    return ESP_OK;
}
...
static esp_err_t validate_image_header(esp_app_desc_t *new_app_info) {
    ...
    ESP_LOGD(TAG, "Current partition %s type %d subtype %d (offset 0x%08x)", running->label, running->type,
             running->subtype, running->address);
    const esp_partition_t *update = esp_ota_get_next_update_partition(NULL);
    ESP_LOGD(TAG, "Update partition %s type %d subtype %d (offset 0x%08x)", update->label, update->type,
        ...
        case HTTP_EVENT_ON_DATA:
            if (!esp_http_client_is_chunked_response(evt->client)) {
                ...
                if (output_pos == 0) {
                    ...
                    ESP_ERROR_CHECK(
                        esp_partition_erase_range(handle->storage_partition, 0, handle->storage_partition->size));
                ...
                if ((progress % 5 == 0) && (progress != last_progress)) {
                    ...
                    ESP_ERROR_CHECK(esp_event_post(Github_OTA_EVENTS, Github_OTA_EVENT_STORAGE_UPDATE_PROGRESS,
                                                   &progress, sizeof(progress), portMAX_DELAY));
...
esp_err_t Github_OTA_storage_update(Github_OTA_client_handle_t *handle) {
    ...
    handle->storage_partition = esp_partition_find_first(ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_ANY,
    ...
    ESP_LOGD(TAG, "Storage Partition %s - Type %x Subtype %x Found at %x - size %d", handle->storage_partition->label,
             handle->storage_partition->type, handle->storage_partition->subtype, handle->storage_partition->address,
             handle->storage_partition->size);
    ...
    if (err == ESP_OK) {
        ...
        ESP_ERROR_CHECK(
            esp_event_post(Github_OTA_EVENTS, Github_OTA_EVENT_FINISH_STORAGE_UPDATE, NULL, 0, portMAX_DELAY));
    } else {
        ESP_LOGE(TAG, "HTTP GET request failed: %s", esp_err_to_name(err));
        ESP_ERROR_CHECK(
            esp_event_post(Github_OTA_EVENTS, Github_OTA_EVENT_STORAGE_UPDATE_FAILED, NULL, 0, portMAX_DELAY));
    }
    esp_http_client_cleanup(client);
    xSemaphoreGive(Github_OTA_lock);
    return ESP_OK;
...
esp_err_t Github_OTA_update(Github_OTA_client_handle_t *handle) {
    ...
    while (1) {
        ...
        if ((progress % 5 == 0) && (progress != last_progress)) {
            ESP_ERROR_CHECK(esp_event_post(Github_OTA_EVENTS, Github_OTA_EVENT_FIRMWARE_UPDATE_PROGRESS, &progress,
                                           sizeof(progress), portMAX_DELAY));
    ...
    if (handle) {
        if (Github_OTA_check(handle) == ESP_OK) {
            if (semver_gt(handle->latest_version, handle->current_version) == 1) {
                ...
                ESP_ERROR_CHECK(
                    esp_event_post(Github_OTA_EVENTS, Github_OTA_EVENT_NOUPDATE_AVAILABLE, NULL, 0, portMAX_DELAY));
            }
        } else {
