...
bool MDNSHandler::startMDNS()
{
	...
	stateManager->setState(MDNSState_e::MDNSState_Starting);
        MDNS.addService(("_" + mdnsConfig->hostname).c_str(), "_tcp", 80);
        char port[20];
	//! Add service needs leading _ on ESP32 implementation for some reason (according to the docs)
	MDNS.addServiceTxt(("_" + mdnsConfig->hostname).c_str(), "_tcp", "stream_port", "80"); //! convert int to const char* using a very efficient implemenation of itoa
