...
LEDManager::ledStateMap_t LEDManager::ledStateMap = {
	...
	{LEDStates_e::_Camera_Error, {{1, 5000}}}, // this also works as a more general error - something went critically wrong? We go here
	{LEDStates_e::_WiFiState_Connecting, {{1, 100}, {0, 100}}},
	{LEDStates_e::_WiFiState_Connected, {{1, 100}, {0, 100}, {1, 100}, {0, 100}, {1, 100}, {0, 100}, {1, 100}, {0, 100}, {1, 100}, {0, 100}}}};
...
void LEDManager::handleLED()
{
	...
	// !TODO what if we want a looping state? Or a state that needs to stay bright? Am overthinking this, aren't I?

	// we've reached the timeout on that state, check if we can grab next one and start displaying it,
	// or if we need to keep displaying the current one
	if (this->currentPatternIndex > this->ledStateMap[this->currentState].size() - 1)
	{
		auto nextState = this->_stateManager->getCurrentState();
		// we want to keep displaying the same state only if its an keepAlive one, but we should change if the incoming one
		// is also an errours state, maybe more serious one this time <- this may be a bad idea
		if ((
				std::find(this->keepAliveStates.begin(), this->keepAliveStates.end(), this->currentState) != this->keepAliveStates.end() ||
				std::find(this->keepAliveStates.begin(), this->keepAliveStates.end(), nextState) != this->keepAliveStates.end()) ||
			(this->currentState != nextState && this->ledStateMap.find(nextState) != this->ledStateMap.end()))
		{
		...
		{
			// it wasn't a keepAlive state, nor did we have another one ready, we're done for now
			this->toggleLED(false);
			return;
		}
	}
